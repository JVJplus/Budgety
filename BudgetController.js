...
BudgetController = (function () {
    ...
    return {
        ...
        restoreLocalStorage: function () {
            var prevData = JSON.parse(localStorage.getItem("budget-data"));
            return prevData.allItems;
        },
        ...
        addItem: function (inputs) {
            ...
            if (inputs.type == "inc") {
                ...
                item = new Income(id, inputs.value, inputs.desc?inputs.desc:inputs.description);
                data.totals["inc"] += inputs.value;
            } else {
                id = generateID(data.allItems.exp);
                item = new Expense(id, inputs.value, inputs.desc?inputs.desc:inputs.description);
